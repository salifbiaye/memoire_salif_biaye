...
la plateforme, puis l'architecture fonctionnelle de I-SIB, les choix technologiques,
...
spécialisés qui communiquent entre eux de manière contrôlée.
\newpage{}
% ─────────────────────────────────────────────────
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/contenu/chapitre-3/architecture_fonctionnelle.png}
...
Les interfaces web de la plateforme sont développées avec \textbf{Next.js 16} et
\textbf{TypeScript}, afin de construire des applications front-office et back-office
modernes, maintenables et adaptées aux différents profils utilisateurs. Le backend repose
sur \textbf{Spring Boot 3.5}, utilisé comme socle de développement des microservices
métier. L'accès aux services est centralisé par \textbf{Spring Cloud Gateway}, qui assure
le routage, les filtres de sécurité et la résilience. La gestion des identités et des
rôles est confiée à \textbf{Keycloak}, tandis que \textbf{Netflix Eureka} permet la
...
des données. \textbf{Redis} est utilisé pour le cache et le contrôle de débit, tandis que
\textbf{RabbitMQ} prend en charge la communication asynchrone entre les services,
notamment pour les événements d'audit. Les documents clients et les pièces KYC sont
stockés dans \textbf{MinIO}, qui permet de gérer un stockage objet sécurisé.
...
segmentation. La traçabilité technique est assurée par \textbf{Zipkin}, qui permet de
suivre les appels entre microservices. Enfin, \textbf{Docker} et \textbf{Docker Compose}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/contenu/chapitre-3/architecture_technique.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/contenu/chapitre-3/workflow_creation_prospect.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/contenu/chapitre-3/workflow_conversion_prospect.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/contenu/chapitre-3/conception_gestion_prospects.png}
...
La vue logique de l'architecture doit être complétée par une vue de déploiement.
Cette seconde lecture montre comment les composants sont réellement organisés en
production, comment les réseaux Docker sont séparés et dans quel ordre les services
doivent être démarrés.

En production, la plateforme I-SIB est décomposée en quatre stacks Docker Compose
indépendants. Cette organisation suit une logique de séparation des responsabilités
au niveau infrastructurel : chaque stack peut être redémarré, mis à jour ou
répliqué sans impacter les autres. Les stacks \texttt{infra}, \texttt{core},
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/contenu/chapitre-3/architecture_deploiement.png}
    \caption{Architecture de déploiement -- 4 stacks Docker Compose en production}
    \label{fig:architecture_deploiement}
\end{figure}

Cette organisation impose un ordre de démarrage précis. La stack \texttt{infra}
doit être lancée en premier afin de rendre disponibles la passerelle, Eureka,
Keycloak, Redis et RabbitMQ. La stack \texttt{core} peut ensuite démarrer les
services de base. La stack \texttt{module} vient après, car elle dépend des
référentiels et de l'infrastructure. Enfin, la stack \texttt{office} est lancée
...
effective de cette architecture à travers les résultats d'implémentation obtenus.
