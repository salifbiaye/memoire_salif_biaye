...
\textbf{La couche de présentation} contient les interfaces utilisées pour accéder à la plateforme. Le \textbf{Front-Office} est destiné aux clients de la banque, tandis que le \textbf{Back-Office} est utilisé par les employés.
...
\textbf{La couche de sécurité et de routage} contrôle l'accès à la plateforme et assure l'acheminement des requêtes vers les différents services. Le composant \textbf{Authentification et accès} gère l'identification des utilisateurs ainsi que la vérification de leurs autorisations. La \textbf{Passerelle de communication} constitue le point d'entrée des applications et redirige les requêtes vers les microservices concernés.

Le \textbf{Registre des services} indique à la passerelle l'emplacement de chaque microservice disponible.

\medskip

\textbf{La couche métier} regroupe les modules qui assurent les principales fonctionnalités de la plateforme : \textbf{CRM}, \textbf{SGI}, \textbf{gestion des utilisateurs}, \textbf{gestion des comptes et des transactions}, \textbf{gestion des agences}, \textbf{comptabilité}, \textbf{trésorerie} et \textbf{crédit}. Le \textbf{service d'audit} enregistre les actions importantes afin d'assurer leur traçabilité.

\medskip

\textbf{La couche de données et de services techniques} regroupe les composants nécessaires au stockage des données et aux services techniques. La \textbf{base de données} conserve les informations, le \textbf{cache} accélère les accès aux données, la \textbf{messagerie} facilite les échanges entre les services et le \textbf{stockage objet} héberge les fichiers et les documents.

\medskip

\textbf{La couche d'infrastructure et d'exploitation} rassemble les outils nécessaires au déploiement, à l'exploitation et au suivi de la plateforme. Les modules peuvent s'appuyer sur des \textbf{services d'intelligence artificielle} selon leurs besoins. Les outils d'\textbf{observabilité} permettent de détecter les anomalies et de superviser le fonctionnement de la plateforme, tandis que la chaîne \textbf{CI/CD} automatise la construction, les tests et le déploiement des applications.
...
Les fonctionnalités d'intelligence artificielle reposent sur des services dédiés à l'analyse et à l'évaluation des prospects.
...
\textit{control} et les données métier \textit{entity}, selon les stéréotypes
de l'approche Entity-Control-Boundary introduite par Jacobson
\cite{jacobson_oose}.

\begin{figure}[H]
    \centering
    \MemoireFigureBox{width=\linewidth,keepaspectratio}{\textwidth}{images/contenu/chapitre-3/conception_creation_prospect.png}
