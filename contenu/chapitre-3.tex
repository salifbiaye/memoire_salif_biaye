...
passant par la passerelle API, le service CRM et sa persistance, le scoring par
...
Le service CRM assure le traitement métier et la persistance associée, tandis
que les services transversaux interviennent pour le scoring et l'audit.
...
La conversion d'un prospect intervient lorsque le chargé de clientèle décide de
transformer un prospect en client.
Le service Relation Client, qui porte les traitements CRM, vérifie l'éligibilité du
prospect, sollicite le service de segmentation,
...
Les opérations principales sont exposées sous forme d'API REST sécurisées et
...
GET    & {\scriptsize\texttt{/api/crm/prospects/my-prospects}}     & Afficher les prospects affectés \\ \hline
GET    & {\scriptsize\texttt{/api/crm/prospects/\{id\}}}           & Consulter la fiche détaillée d'un prospect \\ \hline
PATCH  & {\scriptsize\texttt{/api/crm/prospects/\{id\}}}           & Mettre à jour le prospect ou son étape \\ \hline
POST   & {\tiny\texttt{/api/crm/prospects/\{id\}/activities}} & Ajouter une interaction ou relance \\ \hline
POST   & {\tiny\texttt{/api/crm/prospects/\{id\}/convert}}   & Convertir un prospect en client \\ \hline
\end{tabularx}
\end{table}

\subsection{Vue de conception de la fonctionnalité}

La figure~\ref{fig:conception_gestion_prospects} complète le workflow en
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\textwidth]{images/contenu/chapitre-3/conception_gestion_prospects.png}
