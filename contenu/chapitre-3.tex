...
l'organisation des composants en ensembles Compose. Lors de la validation sur
VPS AlmaLinux, l'exécution a toutefois été réalisée avec \textbf{Podman}
\cite{podman_docs} et \textbf{Podman Compose} \cite{podman_compose_docs}, en
raison de difficultés réseau rencontrées avec Docker dans cet environnement.
...
La plateforme réalisée est déployée en validation sur un VPS AlmaLinux. Cette
vue complète l'architecture logique en présentant l'organisation des ensembles
Compose, les réseaux de conteneurs et le principe de redéploiement progressif.
...
avec Docker Compose en local \cite{docker_compose_docs}, tandis que l'exécution
sur le VPS s'appuie sur Podman \cite{podman_docs} et Podman Compose
\cite{podman_compose_docs}. Les quatre ensembles isolent respectivement les
services communs, les référentiels, les modules métier et les interfaces
utilisateur.
...
La figure~\ref{fig:architecture_deploiement} synthétise cette organisation de
validation.
...
environnement sert à la validation progressive de la solution et reste distinct
de la production.
% ─────────────────────────────────────────────────
\section*{Conclusion}
\phantomsection
