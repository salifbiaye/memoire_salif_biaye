    ...
    \begin{tabular}{|>{\bfseries}p{3.8cm}|p{10.5cm}|}
        ...
        \begin{minipage}[t]{\linewidth}\vspace{2pt}
            \begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=2pt,parsep=0pt]
                ...
                \item Un score de conversion est calculé à partir des informations disponibles.

                \item Le score obtenu, la probabilité de conversion et le niveau de priorité sont ajoutés à la fiche du prospect.

                \item La fiche du prospect est affichée avec les informations enregistrées.
            \end{enumerate}
            \vspace{2pt}\end{minipage}
        \\ \hline
        ...
        Scénario alternatif &
        \begin{minipage}[t]{\linewidth}\vspace{2pt}
            \textbf{A1 -- Email déjà utilisé :} la création est refusée car un prospect est déjà associé à cette adresse email.
            \vspace{2pt}\end{minipage}
        \\ \hline

        Scénario d'exception &
        \textbf{E1 -- Service de scoring indisponible :}
        le score de conversion ne peut pas être calculé. Le prospect est néanmoins créé avec son score initial et l'incident est journalisé.
        \\ \hline

        Postconditions &
        Le prospect est créé, rattaché au conseiller concerné et positionné au stade \textit{LEAD}. Une entrée correspondante est ajoutée au journal d'audit.
        \\ \hline
...
\paragraph{Sécurité}
Les fonctionnalités accessibles dépendent du rôle de l'utilisateur. Un client accède uniquement à ses propres informations, tandis que les collaborateurs disposent des droits nécessaires à leurs missions.

\paragraph{Confidentialité}
Les informations personnelles, bancaires, KYC et les données d'investissement ne doivent être consultées que par les personnes autorisées.

\paragraph{Traçabilité}
Les opérations importantes, telles que les validations d'ordres, les modifications de données ou les changements de statut, doivent laisser une trace consultable.

\paragraph{Disponibilité}
Les utilisateurs doivent pouvoir accéder au système et consulter les informations nécessaires à leurs activités sans interruption prolongée.

\paragraph{Évolutivité}
L'ajout de nouveaux modules ou de nouvelles fonctionnalités ne doit pas nécessiter une refonte importante du système existant.

\paragraph{Ergonomie}
Les écrans doivent être faciles à comprendre et à utiliser afin de limiter les erreurs de manipulation.
